...
export default {
  ...
  async asyncData() {
    const { data } = await axios.get('/display-mikro/capaian-kinerja-sub-kegiatan')
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/display-mikro/capaian-kinerja-sub-kegiatan/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
        ...
        link.href = url
        link.setAttribute('download', 'Capaian Kinerja Sub Kegiatan.xlsx')
...
<template>
  <b-card>
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy.doFilter" striped hover responsive bordered>
