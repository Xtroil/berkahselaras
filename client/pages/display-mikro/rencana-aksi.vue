...
export default {
  ...
  async asyncData() {
    const { data } = await axios.get('/display-mikro/rencana-aksi')
  ...
  methods: {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/display-mikro/rencana-aksi/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
        ...
        link.href = url
        link.setAttribute('download', 'Rencana Aksi Perangkat Daerah.xlsx')
  ...
  <b-card>
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy.doFilter" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        ...
        <b-tr>
          <b-th style="top:90px" v-for="n of $role.isSuper() || $role.isSetda() ? 25 : 24" :key="n">{{ n }}</b-th>
        </b-tr>
      </b-thead>
      <b-tbody>
        <b-tr class="text-center" v-if="!data.data.length">
          <b-td :colspan="$role.isSuper() || $role.isSetda() ? 25 : 24">There are no records to show</b-td>
        </b-tr>
        <b-tr v-else v-for="(item, index) of data.data" :key="item.id">
          ...
          <b-td>{{ item.sasaran_strategis_rpjmd ? item.sasaran_strategis_rpjmd[$helper.getKeyTahun('target')] : '-' }}</b-td>
