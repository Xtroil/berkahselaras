...
export default {
  ...
  async asyncData() {
    const { data } = await axios.get('kinerja-sub-kegiatan')
  ...
  methods: {
    destroy(index, id) {
      doDestroy({
        preConfirm: async () => {
          return axios.delete(`/kinerja-sub-kegiatan/${id}`)
            .then(() => {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/kinerja-sub-kegiatan/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
  ...
  <b-card>
    <div class="text-right">
      ...
      <template v-if="$role.isSuper() || $role.isPerangkatDaerah()" >
        <nuxt-link to="/kinerja-sub-kegiatan/create" class="btn btn-primary">
    ...
    <div class="mt-3">
      <b-row>
        <b-col sm="6" md="3">
          ...
          <FilterSasaranKinerja v-model="filter.sasaran" model="kinerja-sub-kegiatan" />
        </b-col>
        <b-col sm="6" md="3">
          <FilterIndikatorKinerja v-model="filter.indikator" model="kinerja-sub-kegiatan" />
    ...
    <b-table responsive hover striped sticky-header="calc(100vh - 300px)" :fields="table.fields" :items="data.data" :busy="isBusy.doFilter" show-empty class="table-bordered mt-2" head-variant="info">
      ...
      <template #cell(action)="row">
        <div class="text-nowrap">
          <nuxt-link :to="`/kinerja-sub-kegiatan/${row.item.id}`" class="btn btn-outline-success btn-sm m-1 rounded-circle" title="Detail">
            <i class="fa fa-eye" aria-hidden="true"></i>
          </nuxt-link>
          <nuxt-link v-if="($role.isSuper() || $role.isPerangkatDaerah()) && canEditValidasiPerencanaan" :to="`/kinerja-sub-kegiatan/${row.item.id}/edit`" class="btn btn-outline-warning btn-sm m-1 rounded-circle" title="Edit">
          ...
          <template v-if="$role.isSuper()">
            <b-button @click="$refs.validasiSkp.validate('kinerja-sub-kegiatan', row.item.id, row.item.skp_exists)" :variant="row.item.skp_exists ? 'success' : 'outline-primary'" size="sm" class="m-1 rounded-circle"  :title="row.item.skp_exists ? 'Validasi Ulang' : 'Validasi'">
              <i class="fa fa-check" aria-hidden="true"></i>
            </b-button>
            <b-button v-if="!row.item.skp_exists" @click="$refs.validasiSkp.reject('kinerja-sub-kegiatan', row.item.id)" variant="danger" size="sm" class="m-1 rounded-circle"  title="Data tidak sesuai">
