...
export default {
  ...
  methods: {
    async getData(page = 1) {
      ...
      const { data } = await axios.get('/public-display/display-makro/capaian-kinerja-kegiatan', {
        params: {
          page,
  ...
  <b-card>
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        ...
        <b-tr>
          <b-th style="top:40px" v-for="n of 12" :key="n">{{ n }}</b-th>
          <b-th style="top:40px">Target (Rp)</b-th>
          <b-th style="top:40px">Realiasasi (Rp)</b-th>
          <b-th style="top:40px">Capaian (%)</b-th>
        </b-tr>
        <b-tr>
          <b-th style="top:100px" v-for="n of 24" :key="n">{{ n }}</b-th>
        </b-tr>
      </b-thead>
      <b-tbody>
