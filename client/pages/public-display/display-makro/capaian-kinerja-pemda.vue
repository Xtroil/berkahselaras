      ...
      <b-tbody>
        ...
        <template v-else v-for="(bySasaran, sasaranIndex) of data">
          <tr v-for="(item, index) of bySasaran" :key="item.id">
            ...
            <b-td class="text-center">
              <span v-if="!compareRataNasional(item.rata_nasional, item[$helper.getKeyTahunDinamisPublic('capaian', filter.tahun_kinerja)], item.indikator_sasaran_strategis_id).text">Belum ada data dari rata-rata nasional atau capaian</span>
              <span v-else>{{ compareRataNasional(item.rata_nasional, item[$helper.getKeyTahunDinamisPublic('capaian', filter.tahun_kinerja)], item.indikator_sasaran_strategis_id).text }} <span class="font-weight-bold" :class="compareRataNasional(item.rata_nasional, item[$helper.getKeyTahunDinamisPublic('capaian', filter.tahun_kinerja)], item.indikator_sasaran_strategis_id).class">{{ compareRataNasional(item.rata_nasional, item[$helper.getKeyTahunDinamisPublic('capaian', filter.tahun_kinerja)], item.indikator_sasaran_strategis_id).nilai | decimalDigit}}</span> dari <br> Rata-Rata Nasional </span>
            </b-td>
...
export default {
  ...
  methods:{
    ...
    compareRataNasional(rataNasional, capaian, indikatorId) {
      if (!rataNasional || !capaian) return {
        text: null,
      }

      // hardcode Persentase Penduduk Miskin (persen)
      if (indikatorId == 3) {
        return {
          text: 'Lebih rendah',
          nilai: 1.74,
