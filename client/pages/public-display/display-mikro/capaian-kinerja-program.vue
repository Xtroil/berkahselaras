...
export default {
  ...
  methods: {
    async getData(page = 1) {
      ...
      const { data } = await axios.get('/public-display/display-mikro/capaian-kinerja-program', {
        params: {
          filter: this.filter,
          page,
        }
    ...
    <b-table-simple sticky-header="calc(100vh - 200px)" :aria-busy="isBusy" hover responsive bordered striped>
      <b-thead class="text-center align-middle" head-variant="info">
        <b-tr>
          ...
          <b-th class="align-middle" rowspan="2">Sasaran Program</b-th>
        ...
        <b-tr>
          <b-th style="top:110px;" v-for="n of 25" :key="n">{{ n }}</b-th>
        </b-tr>
      </b-thead>
      <b-tbody>
        ...
        <b-tr v-else v-for="(item, index) of data.data" :key="item.id">
          ...
          <b-td>{{ item ? item.program.nama : '-' }}</b-td>
