...
export default {
  ...
  async asyncData() {
    const { data } = await axios.get('kinerja-program')
  ...
  data() {
    return {
      table: {
        fields: [
          { key: 'no', label: 'No' },
          { key: 'sasaran_strategis_pd', label: 'Sasaran Strategis PD' },
          { key: 'satker_iku', label: 'IKU PD' },
      ...
      isBusy: {
        doFilter: false,
        exportExcel: false,
        submitValidasiPerencanaan: false,
      },
    }
  },
  ...
  computed: {
    ...mapGetters({
      user: 'auth/user',
      validasiPerencanaan: 'validasi-perencanaan/data',
  ...
  methods: {
    destroy(index, id) {
      doDestroy({
        preConfirm: async () => {
          return axios.delete(`/kinerja-program/${id}`)
            .then(() => {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/kinerja-program/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
        ...
        link.href = url
        link.setAttribute('download', 'Kinerja Program.xlsx')
    ...
    async submitValidasiPerencanaan() {
      try {
        this.isBusy.submitValidasiPerencanaan = true

        await axios.post(`validasi-perencanaan`)

        Swal.fire('Berhasil', 'Validasi Perencanaan Berhasil', 'success')
        await this.$store.dispatch('validasi-perencanaan/getStatus') // update validasi perencanaan
      } catch (error) {
        throw error
      } finally {
        this.isBusy.submitValidasiPerencanaan = false
      }
    },
    confirmSubmitValidasiPerencanaan() {
      Swal.fire({
        title: 'Submit Validasi Perencanaan',
        text:  'Apakah Anda yakin akan submit data? Data yang sudah di submit tidak dapat diubah.',
        type: 'warning',
      ...
      .then(({value}) => {
        if (value) {
          this.submitValidasiPerencanaan()
        }
      })
    }
  },
...
<template>
  <b-card>
    <b-alert v-if="validasiPerencanaan?.catatan" variant="danger" show>
      <h6 class="alert-heading">Catatan Validasi Perencanaan</h6>
      <p style="white-space: break-spaces;">{{ validasiPerencanaan.catatan }}</p>
    </b-alert>

    <div class="text-right">
      <b-button v-if="canEditValidasiPerencanaan" variant="warning" @click="confirmSubmitValidasiPerencanaan" :disabled="isBusy.submitValidasiPerencanaan">
        <b-spinner small v-if="isBusy.submitValidasiPerencanaan"></b-spinner>
        Submit Validasi Perencanaan
      </b-button>
      ...
      <template v-if="($role.isSuper() || $role.isPerangkatDaerah()) && canEditValidasiPerencanaan" >
        <nuxt-link to="/kinerja-program/create" class="btn btn-primary">
    ...
    <b-row class="mt-3">
      <b-col sm="6" md="4">
        ...
        <FilterSasaranKinerja v-model="filter.sasaran" model="kinerja-program" />
      </b-col>
      <b-col sm="6" md="4">
        <FilterIndikatorKinerja v-model="filter.indikator" model="kinerja-program" />
      ...
      <b-col sm="6" md="4">
        ...
        <FilterPengampuUnitKerja v-if="filter.pengampu == 'unit-kerja' && filter.satuan_kerja_id" v-model="filter.v_struktur_organisasi_id" :satuan-kerja-id="filter.satuan_kerja_id" type="kinerja-program" />
      </b-col>
    </b-row>
    ...
    <b-table responsive hover striped sticky-header="calc(100vh - 300px)" :fields="table.fields" :items="data.data" :busy="isBusy.doFilter" show-empty class="table-bordered mt-2" head-variant="info">
      ...
      <template #cell(action)="row">
        <div class="text-nowrap">
          <nuxt-link :to="`/kinerja-program/${row.item.id}`" class="btn btn-outline-success btn-sm m-1 rounded-circle" title="Detail">
            <i class="fa fa-eye" aria-hidden="true"></i>
          </nuxt-link>
          <nuxt-link v-if="($role.isSuper() || $role.isPerangkatDaerah()) && canEditValidasiPerencanaan" :to="`/kinerja-program/${row.item.id}/edit`" class="btn btn-outline-warning btn-sm m-1 rounded-circle" title="Edit">
          ...
          <template v-if="$role.isSuper()">
            <b-button @click="$refs.validasiSkp.validate('kinerja-program', row.item.id, row.item.skp_exists)" :variant="row.item.skp_exists ? 'success' : 'outline-primary'" size="sm" class="m-1 rounded-circle"  :title="row.item.skp_exists ? 'Validasi Ulang' : 'Validasi'">
              <i class="fa fa-check" aria-hidden="true"></i>
            </b-button>
            <b-button v-if="!row.item.skp_exists" @click="$refs.validasiSkp.reject('kinerja-program', row.item.id)" variant="danger" size="sm" class="m-1 rounded-circle"  title="Data tidak sesuai">
              <i class="fa fa-times" aria-hidden="true"></i>
            </b-button>
          </template>
          <nuxt-link v-if="$role.isPerangkatDaerah() && canEditValidasiPerencanaan" :to="`/kinerja-program/${row.item.id}/kinerja-kegiatan-cross`" class="btn btn-outline-primary btn-sm m-1 rounded-circle" title="Kinerja Cross">
