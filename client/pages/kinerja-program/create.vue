  ...
  <b-card>
    <form @submit.prevent="store()">
      ...
      <div v-if="form.satuan_kerja_id">
        ...
        <b-form-group label-cols="12" label-cols-md="2" label="IKU PD" label-class="font-weight-bold" label-for="sasaran-strategis-pd">
          <v-select id="sasaran-strategis-pd" 
...
export default {
  ...
  async asyncData() {
    const { data: {
      ...
      vso,
      timKerja,
    }} = await axios.get('kinerja-program/create')
  ...
  computed: {
    ...
    selectedSatker: function () {
      if (!this.form.satuan_kerja_id) return 'yang dipilih';

      const satker = this.satker.find(el => el.satuan_kerja_id == this.form.satuan_kerja_id)

      if (!satker) return 'yang dipilih'
  ...
  methods: {
    ...
    store() {
      ...
      axios.post('kinerja-program', this.form)
      .then(() => {
        Swal.fire({
          type: 'success',
          title: 'Berhasil simpan data'
        }).then(() => this.$router.push('/kinerja-program'))
      }).catch(() => {
    ...
    async getSasaranStrategis(satkerId) {
      this.isSasaranStrategisBusy = true
      ...
      this.isSasaranStrategisBusy = false
    },
    setTimKinerja(data) {
  ...
  watch: {
    'form.satuan_kerja_id': function (newVal) {
      if (!this.$role.isSuper() || this.isSasaranStrategisBusy) return false;

      // reset sasaran strategis & iku saat ubah satuan kerja
      this.form.sasaran_strategis_pd_id = null
      this.form.satker_iku_id = null
