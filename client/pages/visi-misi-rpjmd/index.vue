...
export default {
  ...
  async asyncData({ $role }) {
    const { data } = await axios.get('visi-misi-rpjmd')
  ...
  methods: {
    destroy(index, id) {
      doDestroy({
        preConfirm: async () => {
          return axios.delete(`/visi-misi-rpjmd/${id}`)
            .then(() => {
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/visi-misi-rpjmd/export', {
          params: {
            filter: this.filter,
          },
          responseType: 'blob'
        ...
        link.href = url
        link.setAttribute('download', 'Visi, Misi & Tujuan RPJMD.xlsx')
  ...
  <b-card>
    <div class="text-right">
      ...
      <template v-if="canCrud">
        <nuxt-link to="/visi-misi-rpjmd/create" class="btn btn-primary">
