...
export default {
  ...
  async asyncData({ params, redirect, store, query, $role }) {
    ...
    if (![1, 2, 3, 4].includes(triwulan)) {
      redirect('/rapor-kinerja/1/diagram')
      return false
    }
  ...
  watch: {
    triwulan: function(val) {
      this.$router.push({
        path: `/rapor-kinerja/${val}/diagram`,
        query: {
    ...
    <b-card class="mb-2">
      <b-row>
        ...
        <b-col v-if="$role.isSuper()">
          <FilterSatuanKerja v-if="$role.isSuper()" v-model="satuanKerjaId" labelTitle="Satuan Kerja" />
        </b-col>
      </b-row>
    </b-card>

    <DiagramRaporKinerja v-if="satuanKerjaId" :key="key" :triwulan="triwulan" :satuanKerjaId="satuanKerjaId" />
  </div>
</template>