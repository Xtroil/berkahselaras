...
export default {
  ...
  data() {
    return {
      ...
      STATUS_IN_ASSESSMENT: 'in-assessment', // PD sudah submit validasi tapi belum atau sedang dinilai validasi tahap 1
      STATUS_DONE: 'done', // Sudah dinilai validasi tahap 1
      STATUS_IN_ASSESSMENT_2: 'in-assessment-2', // PD sudah submit validasi tapi belum atau sedang dinilai validasi tahap 2
      STATUS_DONE_2: 'done-2', // Sudah dinilai validasi tahap 2
  ...
  computed: {
    isDisablePenilaian() {
      return [this.STATUS_NULL, this.STATUS_DONE_2].includes(this.statusPenilaian)
    },
    ...mapGetters({
      user: 'auth/user'
    })
  ...
  methods: {
    async getData() {
      ...
      const { data: {data: komponenList, statusPenilaian, penilaianKomponen} } = await axios.get('/lke/penilaian-2', {
        params: this.filter,
      })
    ...
    async close() {
      ...
      try {
        const { data: { status }} = await axios.post('/lke/penilaian-2/close', this.filter)
    ...
    async cancelClose() {
      ...
      try {
        const { data: { status }} = await axios.post('/lke/penilaian-2/cancel-close', this.filter)
    ...
    async exportExcel() {
      ...
      try {
        const { data } = await axios.get('/lke/penilaian-2/export', {
          params: this.filter,
          responseType: 'blob',
        })
        ...
        link.href = url
        link.setAttribute('download', 'Penilaian LKE Tahap Akhir.xlsx')
...
<template>
  <b-card>
    ...
    <div v-else-if="filter.satuan_kerja_id">
      ...
      <div class="text-right" v-if="statusPenilaian == STATUS_IN_ASSESSMENT_2">
        <b-button v-if="statusPenilaian == STATUS_IN_ASSESSMENT_2" variant="success" @click="closeConfirm()" :disabled="isBusy.close">
      ...
      <template v-if="statusPenilaian == STATUS_DONE_2">
      ...
      <b-table-simple v-else class="mt-3 pb-5" responsive bordered hover>
        ...
        <b-tbody v-if="isReady">
          <template v-for="(komponen, komponenIndex) in komponenList">
            <b-tr :key="komponenIndex">
              <b-td colspan="6"><h5>{{ komponenIndex + 1 }}. {{ komponen.nama }}</h5></b-td>
            </b-tr>

            <template v-for="(subKomponen, subKomponenIndex) in komponen.sub_komponen">
              <b-tr :key="`${komponenIndex}-${subKomponenIndex}`">
                <b-td><h6>{{ komponenIndex + 1 }}.{{ subKomponenIndex + 1 }}</h6></b-td>
                <b-td colspan="6"><h6>{{ subKomponen.nama }}</h6></b-td>
              </b-tr>

              <b-tr v-for="(kriteria, kriteriaIndex) in subKomponen.kriteria" :key="`${komponenIndex}-${subKomponenIndex}-${kriteriaIndex}`" :class="{'bg-pink': form[getLastRiwayat(kriteria.eviden.riwayat).id].is_revisi || form[getLastRiwayat(kriteria.eviden.riwayat).id].nilai != getLastRiwayat(kriteria.eviden.riwayat).parameter_id }">
                <b-td>{{ komponenIndex + 1 }}.{{ subKomponenIndex + 1 }}.{{ kriteriaIndex + 1 }}</b-td>
