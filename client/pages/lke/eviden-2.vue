...
export default {
  ...
  async asyncData() {
    const { data: { data: komponenList, statusPenilaian } } = await axios.get('/lke/eviden-2')
  ...
  computed: {
    showHasilPenilaian: function () {
      return [this.STATUS_DONE_2].includes(this.statusPenilaian)
    }
  },
  ...
  methods: {
    ...
    async submitPenilaian() {
      ...
      try {
        const { data: { success, message, status } } = await axios.post('/lke/eviden-2/penilaian')
...
<template>
  <b-card v-if="!komponenList.length">
  ...
  <b-card v-else>
    ...
    <b-alert variant="info" :show="statusPenilaian == STATUS_IN_ASSESSMENT_2">
      Sedang dalam tahap validasi
    </b-alert>
    <b-alert variant="success" :show="statusPenilaian == STATUS_DONE_2">
    ...
    <b-table-simple class="mt-3 pb-5" responsive bordered hover>
      <b-thead>
        <b-tr>
          ...
          <b-th v-if="statusPenilaian == STATUS_DONE_2">Poin</b-th>
        </b-tr>
      </b-thead>
      <b-tbody v-if="isReady">
        <template v-for="(komponen, komponenIndex) in komponenList">
          <b-tr :key="komponen.id">
            ...
            <b-td v-if="statusPenilaian == STATUS_DONE_2">
          ...
          <template v-for="(subKomponen, subKomponenIndex) in komponen.sub_komponen">
            ...
            <b-tr v-for="kriteria in subKomponen.kriteria" :key="kriteria.id" :class="{'bg-pink': showHasilPenilaian && (form[kriteria.id].parameter_id != getLastRiwayat(kriteria.eviden.riwayat).nilai || !getLastRiwayat(kriteria.eviden.riwayat).status) }">
              ...
              <b-td>
                ...
                <span v-else>
                  <b>{{ kriteria.eviden.parameter.nama }}</b>
                  <div v-if="kriteria.eviden.parameter.nama != kriteria.eviden.parameter.keterangan">{{ kriteria.eviden.parameter.keterangan }}</div>
                </span>
              </b-td>
              <b-td>
                ...
                <template v-else>
                  <div v-for="(keterangan, indexEviden) of kriteria.keterangan_eviden" :key="indexEviden">
                    ...
                    <b-form-file
                      v-if="(!statusPenilaian) && !kriteria.is_auto && !keterangan.includes('https')"
                      class="mt-1"
