...
export default {
  ...
  methods: {
    async login () {
      ...
      try {
        const { data: { data } } = await axios.post('/login', this.form)
...
<template>
  <div class="container mt-5" style="max-width: 890px">
    <b-card header="Log In">
      ...
      <center><img
                src="~assets/images/logobangkep1.png"
                style="width: 80px"
                alt=""
              /><br><br></center>
        ...
        <div class="form-group row">
          ...
          <div class="col-md-7">
            <b-form-select v-model="tahunKinerja">
              <b-form-select-option v-for="tahun of $const.tahun_kinerja_list" :key="tahun" :value="tahun">{{ tahun }}</b-form-select-option>
            </b-form-select>
          </div>
        </div>
