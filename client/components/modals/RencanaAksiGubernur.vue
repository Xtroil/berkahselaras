<script>
const componentId = 'modal-rencana-aksi-gubernur'
...
export default {
  ...
  mounted() {
    this.$nuxt.$on(this.showEventId, (satuanKerjaId, sasaranStrategisRpjmdId, iku) => {
  ...
  methods: {
    async getData(page = 1) {
      ...
      const { data } = await axios.get('public-display/rencana-aksi-gubernur', {
        params: {
...
<template>
  <b-modal :id="id" title="Rencana Aksi" size="xl">
      ...
      <b-tbody>
        ...
        <template v-else>
          <b-tr v-for="(rencanaAksi, index) in data.data" :key="index">
            <b-td v-if="index == 0" :rowspan="data.data.length">{{ iku }}</b-td>
            ...
            <b-td v-for="n in 4" :key="n">
              {{ rencanaAksi[`tw${n}`] }} {{ rencanaAksi.satuan }}
            </b-td>
            <b-td>
