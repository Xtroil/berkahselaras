    ...
    >
      <b-thead class="text-center align-middle" head-variant="info">
        <b-tr>
          ...
          <b-th colspan="3"
            >CAPAIAN KINERJA TAHUN {{ $helper.getTahunKinerja() }}</b-th
          >
          <b-th colspan="6">PENINGKATAN DARI TAHUN LALU</b-th>
          <b-th colspan="3"
            >CAPAIAN TAHUN {{ $helper.getTahunKinerja() }} TERHADAP TARGET AKHIR
            RPJMD/Renstra</b-th
          >
          <b-th colspan="2">PERBANDINGAN DENGAN NASIONAL</b-th>
          <b-th rowspan="2"
            >PERINGKAT SULAWESI TENGAH DI LEVEL PULAU SULAWESI</b-th
          >
          <b-th rowspan="2">PERINGKAT SULAWESI TENGAH DI LEVEL NASIONAL</b-th>
          <b-th rowspan="2">IKU PD YANG TERKAIT</b-th>
          <b-th rowspan="2">TARGET TAHUN {{ $helper.getTahunKinerja() }}</b-th>
          <b-th rowspan="2"
            >REALIASI TAHUN {{ $helper.getTahunKinerja() }}</b-th
          >
        ...
        <b-tr>
          ...
          <b-th
            >PENINGKATAN ATAU PENURUNAN REALISASI DARI TAHUN
            {{ $helper.getTahunKinerja() - 1 }}</b-th
          >
          <b-th
            >CAPAIAN KINERJA TAHUN {{ $helper.getTahunKinerja() - 1 }} (%)</b-th
          >
          <b-th>CAPAIAN KINERJA TAHUN {{ $helper.getTahunKinerja() }} (%)</b-th>
          <b-th
            >PENINGKATAN ATAU PENURUNAN CAPAIAN DARI TAHUN
            {{ $helper.getTahunKinerja() - 1 }}</b-th
          >
          ...
          <b-th
            >PERBANDINGAN REALISASI TAHUN {{ $helper.getTahunKinerja() }} DENGAN
            RATA-RATA NASIONAL</b-th
          >
      ...
      <b-tbody>
        ...
        <tr v-for="(item, index) of data.data" :key="item.id">
          ...
          <td>
            {{ item.sasaran_strategis_pd[$helper.getKeyTahun("target")] }}
          </td>
          <td>
            {{
              item.sasaran_strategis_pd[$helper.getKeyTahun("realisasi")] || "-"
            }}
          </td>
          <td>
            {{
              item.sasaran_strategis_pd[$helper.getKeyTahun("capaian")] || "-"
            }}
          </td>

          <td>
            {{
              item.sasaran_strategis_pd[$helper.getKeyTahun("realisasi", -1)] ||
              "-"
            }}
          </td>
          <td>
          ...
          <td>
            {{ item.sasaran_strategis_pd[$helper.getKeyTahun("capaian", -1)] }}
          </td>
          <td>
            {{ item.sasaran_strategis_pd[$helper.getKeyTahun("capaian")] }}
          </td>
          ...
          <td>
            <template
              v-if="
                item.satker_iku[$helper.getKeyTahun('realisasi')] &&
                item.satker_iku.rata_nasional
              "
            >
              {{
                item.satker_iku[$helper.getKeyTahun("realisasi")] /
          ...
          <td>
            <nuxt-link to="/display-mikro/cascading">Cascading</nuxt-link>
          </td>
          <td>{{ item.program.nama }}</td>
          <td>{{ item.anggaran | rupiah }}</td>
          <td>
            <template v-if="item.realisasi_anggaran">{{
              item.realisasi_anggaran | rupiah
            }}</template>
            <template v-else>-</template>
          </td>
          ...
          <td>
            <template v-if="item.realisasi_anggaran">{{
              (item.anggaran - item.realisasi_anggaran) | rupiah
            }}</template>
            <template v-else>-</template>
          </td>
          <td>
            <template v-if="item.realisasi_anggaran">{{
              Math.round(
                (((item.anggaran - item.realisasi_anggaran) / item.anggaran) *
                  100 +
                  Number.EPSILON) *
                  100
              ) / 100
