              ...
              <li v-for="sasaranRpjmd of data" :key="sasaranRpjmd.id">
                <span class="text-left green-box sasaranRpjmd">
                  <div class="text-right w-100">
                    ...
                    <b-button
                      @click="$nuxt.$emit('show-modal-rencana-aksi-gubernur', satkerId, sasaranRpjmd.id, sasaranRpjmd.indikator_sasaran_strategis.indikator)"
                      variant="primary"
                      size="sm"
                      pill
                      v-b-tooltip.hover
                    ...
                    <b-button
                      @click="$nuxt.$emit('show-modal-realisasi-rencana-aksi-gubernur', satkerId, sasaranRpjmd.id, sasaranRpjmd.indikator_sasaran_strategis.indikator)"
                      variant="primary"
                      size="sm"
                      pill
                      v-b-tooltip.hover
                    ...
                    <li :key="`${satkerSasaran.id}-${satkerSasaran.is_cross}`" v-if="showBayangan ? satkerSasaran.render_status != false : true" :class="{cross: satkerSasaran.is_cross, 'cross-exists': satkerSasaran.sasaran_strategis_pd_cross_exists}">
                      <span class="text-left satkerSasaran yellow-box" :class="{'d-none': showBayangan && satkerSasaran.render_status == 'hide'}">
                        ...
                        <div v-if="!satkerSasaran.is_cross" class="text-right w-100">
                          ...
                          <b-button
                            @click="$nuxt.$emit('show-modal-rencana-aksi', satkerId, satkerSasaran.id, satkerSasaran.iku)"
                            variant="primary"
                            size="sm"
                            pill
                            v-b-tooltip.hover
                          ...
                          <b-button
                            @click="$nuxt.$emit('show-modal-realisasi-rencana-aksi', satkerId, satkerSasaran.id, satkerSasaran.iku)"
                            variant="primary"
                            size="sm"
                            pill
                            v-b-tooltip.hover
                        ...
                        <div v-if="visibility.indikator" class="mt-2 d-flex align-items-baseline w-100">
                          ...
                          <div>
                            <small>{{ labels.lv2.indikator }}:</small>
                            <h6 class="m-0  font-weight-bold" style="word-wrap: anywhere;">({{ satkerSasaran.iku }})</h6>
                          </div>
                        </div>
                        <div v-if="visibility.pengampu" class="mt-2 indikator d-flex align-items-baseline w-100">
                          <i class="fa fa-user mr-2"></i>
                      ...
                      <ul v-if="maxLevel >= 3 && satkerSasaran.kinerja_program && satkerSasaran.kinerja_program.length">
                        <template v-for="kinerjaProgram of mergeKinerjaProgram(satkerSasaran.id, satkerSasaran.kinerja_program, satkerSasaran.kinerja_program_cross)">
                          <li :key="`${kinerjaProgram.id}-${kinerjaProgram.is_cross}`" v-if="showBayangan ? satkerSasaran.render_status != false && kinerjaProgram.render_status != false : true" :class="{cross: kinerjaProgram.is_cross, 'cross-exists': kinerjaProgram.kinerja_program_cross_exists}">
                            <span class="text-left kinerjaProgram blue-box" :class="{'d-none': showBayangan && kinerjaProgram.render_status == 'hide'}">
                              ...
                              <div v-if="!kinerjaProgram.is_cross" class="d-flex w-100 justify-content-between mb-2">
                                <template v-if="options.showIconCapaian">
                                  <div>
                              ...
                              <div v-if="visibility.indikator" class="mt-2 indikator d-flex align-items-baseline w-100">
                                ...
                                <div>
                                  <small>{{ labels.lv3.indikator }}:</small>
                                  <h6 h6 class="m-0 indikator font-weight-bold" style="word-wrap: anywhere;">({{ kinerjaProgram.indikator }})</h6>
                                </div>
                              </div>
                            ...
                            <ul v-if="maxLevel >= 4 && kinerjaProgram.kinerja_kegiatan && kinerjaProgram.kinerja_kegiatan.length">
                              <template v-for="kinerjaKegiatan of mergeKinerjaKegiatan(kinerjaProgram.id, kinerjaProgram.kinerja_kegiatan, kinerjaProgram.kinerja_kegiatan_cross)">
                                <li :key="`${kinerjaKegiatan.id}-${kinerjaKegiatan.is_cross}`" v-if="showBayangan ? satkerSasaran.render_status != false && kinerjaProgram.render_status != false && kinerjaKegiatan.render_status != false : true" :class="{cross: kinerjaKegiatan.is_cross, 'cross-exists': kinerjaKegiatan.kinerja_kegiatan_cross_exists}">
                                  <span class="text-left kinerjaKegiatan lightblue-box">
                                    ...
                                    <div v-if="!kinerjaKegiatan.is_cross && options.showIconCapaian" class="d-flex w-100 justify-content-between">
                                      <div>
                                      ...
                                      <div class="text-left level-5" :class="{highlight: options.highlight == `kinerja-sub-kegiatan-${kinerjaSubKegiatan.id}-${kinerjaSubKegiatan.is_cross}`}">
                                        ...
                                        <div v-if="!kinerjaSubKegiatan.is_cross" class="d-flex w-100 justify-content-between">
                                          <template v-if="options.showIconCapaian">
                                            ...
                                            <div>
                                              <i class="fa-stack" @click="showKamusIndikatorModal(kinerjaSubKegiatan.indikator, kinerjaSubKegiatan.indikator_kemendagri)" v-b-tooltip.hover title="Kamus Indikator" style="cursor:pointer">
                                                <i class="fa fa-circle fa-stack-2x text-white"></i>
...
export default {
    ...
    return {
      ...
      labels: {
        ...
        lv3: {
          ...
          indikator: 'Indikator Program (Intermediate Outcome)',
        },
        lv4: {
  ...
  methods: {
    ...
    mergeKinerjaSubKegiatan(kinerjaKegiatanId, kinerjaSubKegiatan, kinerjaSubKegiatanCross) {
