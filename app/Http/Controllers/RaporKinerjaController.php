...
class RaporKinerjaController extends Controller
{
    public function diagram(int $triwulan, Request $request)
    {
        ...
        $rank = $this->calculateRank($triwulan, $satuanKerja->satuan_kerja_id, $tahunKinerja);

        $data = KinerjaSubKegiatan::tahunKinerja($tahunKinerja)
     ...
     */
    private function calculateRank(int $triwulan, string $satkerId, ?int $tahunKinerja = null): array
    {
         ...
         */
        $list = Cache::remember($cacheKey, 60 * 10, function () use ($triwulan, $tahunKinerja) {
            $triwulanList = TRIWULAN_BULAN[$triwulan];
    ...
    public function report(int $triwulan, bool $isExport = false)
    {
        $triwulanList = TRIWULAN_BULAN[$triwulan];
        ...
        $data = KinerjaSubKegiatan::tahunKinerja()
            ...
            ->values()
            ->transform(function ($item, $rank) use ($triwulan, $satkerList, $perubahanJumlahOutputList) {
    ...
    public function reportExport(int $triwulan)
    {
        $data = $this->report($triwulan, true);
