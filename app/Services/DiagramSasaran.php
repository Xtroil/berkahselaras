        ...
        $relations = [
            ...
            [
                ...
                'sasaranStrategisPd.kinerjaProgram' => fn(Builder $query) => $query
                    ->select('id', 'sasaran_strategis_pd_id', 'sasaran', 'indikator', 'program_id', 'v_struktur_organisasi_id', 'pengampu', 'tim_kerja_id', 'target', 'realisasi', 'capaian', 'penyebab_kegagalan', 'kinerja_bayangan_id')
                    ->tahunKinerja()
                    ->with($arrayFilter([
            ...
            [
                ...
                'sasaranStrategisPd.kinerjaProgram.kinerjaKegiatan' => fn(Builder $query) => $query
                    ->select('id', 'kinerja_program_id', 'sasaran', 'indikator', 'kegiatan_id', 'v_struktur_organisasi_id', 'pengampu', 'tim_kerja_id', 'target', 'realisasi', 'capaian', 'penyebab_kegagalan', 'kinerja_bayangan_id')
                    ->tahunKinerja()
                    ->with($arrayFilter([
                ...
                'sasaranStrategisPd.kinerjaProgram.kinerjaKegiatan.kinerjaSubKegiatan' => fn(Builder $query) => $query
                    ...
                    ->with($arrayFilter([
                        ...
                        ! $isDefaultType ? null : 'strukturOrganisasi' => fn(Builder $query) => $query
                            ...
                            ")
                            ->when(
                                $satuanKerjaId == SATKER_DISDUKCAPIL,
                                fn(Builder $query) => $query
                                    ->withoutGlobalScope('active')
                            ),
                        ! $isDefaultType ? null : 'timKerja:id,nama,nip_ketua',
        ...
        $result = self::get($satuanKerjaId, $maxLevel, $type)
            ...
            ->transform(function (SupportCollection $sasaranStrategisRpjmd) {
                ...
                $data['sasaran_strategis_pd'] = $sasaranStrategisPd
                    ->flatten(1)
                    ->groupBy(fn(array $item) => "{$item['sasaran_strategis_satker']}-{$item['iku']}")
